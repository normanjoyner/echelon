        ...
        cluster.on("listening", function(){
            ...
            cluster.on("added_node", function(node){
                if(cluster.is_master()){
                    queue.add_worker(node, node.metadata.concurrency);
                    queue.increase_concurrency(node.metadata.concurrency);
                }
            });
            ...
            cluster.on("removed_node", function(node){
                if(cluster.is_master()){
                    ...
                    queue.decrease_concurrency(node.metadata.concurrency);
                }
            });
